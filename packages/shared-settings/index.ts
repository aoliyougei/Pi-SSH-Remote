...
const REGISTRY_SYMBOL = Symbol.for("@aoliyougei/pi-shared-settings/registry-v1");
const COMMAND_NAME = "aoliyougei-settings";
...
async function showSettingsMenu(ctx: ExtensionContext, registry: SharedSettingsRegistry): Promise<void> {
  ...
  if (sections.length === 0) {
    ctx.ui.notify("No configurable @aoliyougei extension settings are currently available", "info");
    return;
  }
...
function ensureCommand(pi: ExtensionAPI, registry: SharedSettingsRegistry): void {
  ...
  pi.registerCommand(COMMAND_NAME, {
    description: "Configure installed @aoliyougei extensions",
